...
class SupernoteParser:
    ...
    def _decode_ratta_rle_enhanced(self, compressed_data: bytes, width: int, height: int) -> np.ndarray:
        """RATTA_RLE decoder matching supernotelib reference implementation
        
        Based on the working supernotelib decoder with proper:
        - Multi-byte length handling with 0x80 mask logic
        - Special length markers (0xFF -> 0x4000 pixels)
        - Proper color mapping including transparent pixels
        - Pixel fill in correct row-major order
        """
        ...
        logger.debug(f"RATTA_RLE decoder: {len(compressed_data)} bytes for {width}x{height}")
        
        # Color mapping matching supernotelib RattaRleDecoder
        color_map = {
            0x61: 0,      # COLORCODE_BLACK
            0x62: 255,    # COLORCODE_BACKGROUND (transparent -> white)
            0x63: 64,     # COLORCODE_DARK_GRAY  
            0x64: 128,    # COLORCODE_GRAY
            0x65: 255,    # COLORCODE_WHITE
            0x66: 0,      # COLORCODE_MARKER_BLACK
            0x67: 64,     # COLORCODE_MARKER_DARK_GRAY
            0x68: 128,    # COLORCODE_MARKER_GRAY
        }
        ...
        # Process RLE data using supernotelib algorithm
        data_iter = iter(compressed_data)
        holder = ()  # For multi-byte length sequences
        waiting_queue = []  # Queue for deferred processing
        
        try:
            while pixel_idx < max_pixels:
                colorcode = next(data_iter)
                length_byte = next(data_iter)
                data_pushed = False
                ...
                if len(holder) > 0:
                    ...
                    if colorcode == prev_colorcode:
                        # Combine lengths for repeated color
                        length = 1 + length_byte + (((prev_length & 0x7F) + 1) << 7)
                        waiting_queue.append((colorcode, length))
                        data_pushed = True
                    else:
                        # Process held data first
                        prev_length = ((prev_length & 0x7F) + 1) << 7
                        waiting_queue.append((prev_colorcode, prev_length))
                
                if not data_pushed:
                    if length_byte == 0xFF:  # SPECIAL_LENGTH_MARKER
                        # Special case: 0xFF means 16384 pixels
                        length = 0x4000  # SPECIAL_LENGTH = 16384
                        waiting_queue.append((colorcode, length))
                        data_pushed = True
                        ...
                        # Normal length: add 1 and process
                        length = length_byte + 1
                        waiting_queue.append((colorcode, length))
                        data_pushed = True
                
                # Process all queued data
                while waiting_queue and pixel_idx < max_pixels:
                    colorcode, length = waiting_queue.pop(0)
                    color = color_map.get(colorcode, 255)
                    
                    # Fill pixels in row-major order
                    pixels_to_fill = min(length, max_pixels - pixel_idx)
            ...
            # Handle any remaining held data
            if len(holder) > 0:
                colorcode, length_byte = holder
                # Calculate remaining pixels needed
                remaining = max_pixels - pixel_idx
                length = min(remaining, ((length_byte & 0x7F) + 1) << 7)
                if length > 0:
                    color = color_map.get(colorcode, 255)
        ...
        logger.info(f"RATTA_RLE decode: {pixel_idx:,} pixels processed, {non_white:,} non-white pixels")
    ...
    def _flatten_layers(self, layer_images: Dict[str, Image.Image], 
        ...
        CRITICAL FIX: The visibility logic was inverted. INVISIBLE mode 
        continues (skips) layers that are invisible, it doesn't force include them.
        The high pixel count comes from including ALL visible layers properly.
        """
        ...
        # Assume all layers are visible by default (like sn2md does)
        default_visibility = {name: True for name in layer_order}
        
        for layer_name in reversed(layer_order):  # sn2md uses reversed order
            # Get layer visibility
            is_visible = default_visibility.get(layer_name, True)
            overlay = visibility_overlay.get(layer_name, VisibilityOverlay.DEFAULT)
            
            # Apply sn2md visibility logic (from converter.py line 139-140)
            should_skip = False
            if overlay == VisibilityOverlay.INVISIBLE:
                # INVISIBLE overlay: skip this layer (continue statement in sn2md)
                should_skip = True
            elif overlay == VisibilityOverlay.DEFAULT and not is_visible:
                # DEFAULT overlay but layer not visible: skip
                should_skip = True
            elif overlay == VisibilityOverlay.VISIBLE:
                # VISIBLE overlay: always include
                should_skip = False
            else:
                # DEFAULT overlay and layer is visible: include
                should_skip = False
            
            if not should_skip and layer_name in layer_images:
